...
识别帧按约 30 万像素的总预算缩放，而不是分别限制宽 640、高 480。固定宽高上限会让 9:16 竖屏帧被压成约 270×480，却让横屏保留约 640×360，导致旋转手机后识别率突然提高。像素预算算法会让两个方向分别得到约 416×739 和 739×416，计算量接近，细节量也接近。支持相关能力的浏览器还会请求连续自动对焦，以提高缓慢移动手机时捕捉到清晰帧的概率。
...
如果一次识别耗时超过 interval，`setInterval` 会继续排队，最终造成卡顿。`useRecognizer` 用递归 `setTimeout`：本轮完成后才安排下一轮，并用 `busyRef` 再加一道不可重入保护。识别成功前周期为 240ms，成功后降到 1200ms。
