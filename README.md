...
- 支持双指捏合、缩放条和加减按钮；优先使用设备镜头变焦，不支持时自动使用与 OpenCV 同步的数字缩放
- OpenCV.js 脚本、WASM 运行时和参考图库分阶段加载
- 多参考目标预提取特征，扫描过程中不重复计算
- 640 × 480 上限的识别帧、320ms 默认检测周期、不可重入检测
...
- 摄像头是否提供硬件变焦由设备和浏览器共同决定。界面显示“镜头变焦”时使用相机能力；显示“数字变焦”时会同步放大预览和 OpenCV 识别区域。
